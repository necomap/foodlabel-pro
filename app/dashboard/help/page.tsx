...
const FAQS: FAQ[] = [
  ...
  { category: 'レシピ', q: '「メモ」欄と「印字コメント」欄の違いは？', a: '「メモ（自分用・ラベル非印字）」は自分用のメモでラベルには印刷されません。「印字コメント」はシールに印刷されます。' },
  { category: 'レシピ', q: '添加物はどう入力する？', a: '材料入力欄の「添加物」チェックをONにすると添加物として登録されます。ラベルでは「/（スラッシュ）」の後に重量順で表示されます。' },
  { category: 'シール印刷', q: 'シールに住所や電話番号を印刷するには？', a: '設定→店舗管理で住所・電話番号を登録してください。シール印刷画面で「電話番号を表示」にチェックを入れると印刷されます。個人事業主は代表者名の表示義務があります。' },
  { category: 'シール印刷', q: 'シール印刷の表示設定を毎回設定し直す必要がある？', a: 'チェックボックスの設定はブラウザに保存されます。次回アクセス時も前回の設定が維持されます。' },
  { category: 'シール印刷', q: 'ラベルプリンターで印刷するには？', a: 'シール印刷画面で「ラベルプリンタ」を選択し、シールサイズ（幅・高さ mm）を入力します。「印刷する」ボタンでブラウザの印刷ダイアログが開きます。' },
  { category: 'シール印刷', q: 'Bluetoothラベルプリンターは使えますか？', a: 'Windowsにドライバーが入っていれば中国製を含むほとんどのBluetooth・USB接続ラベルプリンターが使えます。プリンターをPCに接続してドライバーをインストール後、ブラウザ印刷で選択できます。' },
  { category: 'シール印刷', q: 'A4用紙に複数のシールを印刷するには？', a: 'シール印刷画面で「A4プリンタ」を選択し、列数・行数・余白を設定します。' },
  ...
  { category: '設定', q: '店舗を複数登録できますか？', a: 'はい。設定→店舗管理から複数追加できます。シール印刷時に店舗を選択すると、その店舗の住所・電話番号がラベルに印字されます。' },
