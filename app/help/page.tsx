...
const FAQS = [
  { q: 'FoodLabel Proとはどんなサービスですか？', a: '食品を販売する事業者向けの食品成分表示ラベル管理システムです。レシピ登録・アレルゲン自動判定・栄養成分計算・ラベル印刷までをワンストップで管理できます。製菓・製パン・総菜・弁当・加工食品など、食品表示が必要なすべての事業者にご利用いただけます。' },
  { q: '無料で使えますか？', a: 'はい、フリープランは無料でご利用いただけます。レシピ10件・ラベル印刷月20枚・店舗1件まで対応しています。より多くの機能が必要な場合は月額980円のプレミアムプランをご利用ください。' },
  ...
  { q: 'どんなプリンターで印刷できますか？', a: 'A4のラベルシール用紙（市販のダイカットラベル用紙）への複数面付け印刷は、レーザープリンターでの動作を確認済みです（フチあり印刷。フチなし印刷にはプリンター側の対応が必要です）。ラベルプリンター本体は機種によって対応状況が異なります。あらかじめ決まったサイズにカット済みの「ダイカットラベル」であれば印刷できることが多い一方、連続ロール紙に長さを指定して印刷する「無定長（可変長）」タイプは現状うまく印刷できない場合があります。また、安価な中国製Bluetooth/USBラベルプリンターの多くは専用アプリでの印刷が前提のため、本サービスでは基本的にご利用いただけません。購入前に、パソコンの標準的なプリンタードライバー経由で印刷できる機種かどうかのご確認をおすすめします。バーコード・識別マーク（プラ・紙などのリサイクルマーク）・ロゴ・QRコードの印字にも対応しています。' },
  { q: '複数の店舗を管理できますか？', a: 'プレミアムプランでは店舗数無制限で管理できます。ラベル印刷時に店舗を選択すると、その店舗の住所・電話番号がラベルに印字されます。' },
