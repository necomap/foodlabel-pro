...
">
  ...
  ${content.barcode && content.showBarcode !== false ? `<div style="text-align:center;margin-top:0.5mm;width:100%;clear:both;">
    <img src="https://barcodeapi.org/api/${getBarcodeApiPath(content.barcode)}/${encodeURIComponent(content.barcode)}?height=${content.barcodeHeightPx ?? 30}${content.showBarcodeText === false ? '&text=none' : ''}" style="width:auto;max-width:100%;height:${content.barcodeHeightMm ?? 7}mm;" onerror="this.style.display='none'" />
  </div>` : ''}
</div>
`;
  // ラベルプリンタ用：ラベルのみ
