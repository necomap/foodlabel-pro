...
// 識別マーク（リサイクルマーク）
// plastic/paper/pet/board は公式マークの実データをトレースしたSVGパス（2026-08 差し替え）。
// steel/aluminum は公式データ未入手のため、枠囲みテキストの仮表示。実データ入手後に差し替えること。
...
const RECYCLE_MARK_DEFS: Record<string, RecycleMarkDef> = {
  ...
  steel:    { kind: 'placeholder', label: 'スチール缶' },
  aluminum: { kind: 'placeholder', label: 'アルミ缶' },
};
